...
fetchChatsSuccess: (state, action: PayloadAction<Chat[]>) => {
  ...
  const validChats = action.payload
    .filter(chat => chat._id && /^[a-f\d]{24}$/i.test(chat._id)); // Valid MongoDB ObjectId
  state.chatHistory = validChats;
  state.isLoading = false;
  if (validChats.length > 0 && !state.currentChatId) {
    state.currentChatId = validChats[0]._id;
  }
},
...
sendMessageSuccess: (state, action: PayloadAction<{ userMessage: Message; assistantMessage: Message; chatId: string; title?: string }>) => {
