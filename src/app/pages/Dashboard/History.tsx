...
import { useGetActivitiesQuery, useDeleteActivityMutation, useClearAllActivitiesMutation } from "../../../Features/auth/authApi";
...
const History: React.FC = () => {
  ...
  if (isLoading) return <div className="p-4 text-gray-600">Loading activities...</div>;
  if (isError)
    return (
      <div className="p-4 text-red-600">⚠️ Could not fetch activities. Please start your backend server.</div>
    );
      ...
      <div className="max-w-4xl mx-auto">
        <header className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500">Your complete activity timeline — reminders, quizzes, chats, and files</p>
          </div>
          {activities.length > 0 && (
            <button
        ...
        {activities.length === 0 ? (
          <div className="border border-dashed border-gray-300 rounded-lg p-8 text-center text-gray-500">
            ...
            <p className="text-sm">Your activities will appear here once you start using the app.</p>
          </div>
        ) : (
          <div className="max-h-[450px] overflow-y-auto">
            <ul className="space-y-4">
              {activities.map((activity) => (
                <li key={activity.id} className="bg-white shadow-sm rounded-lg p-4 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {fmtDate(activity.createdAt)} at {fmtTime(activity.createdAt)}
                    </p>
                    {activity.type === 'reminder' && activity.lastSeen && (
                      <p className="text-sm text-gray-500 mt-1">
                        Last seen: {fmtDate(activity.lastSeen)} at {fmtTime(activity.lastSeen)}
                      </p>
                    )}
                  </div>
                </li>
        ...
        {showClearConfirm && (
          <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <h3 className="text-lg font-semibold text-gray-800 mb-4">Clear All History</h3>
              <p className="text-gray-600 mb-6">Are you sure you want to clear all your activity history? This action cannot be undone.</p>
